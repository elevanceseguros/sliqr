...
import { Zap, Download, RefreshCw, ChevronUp, ChevronDown, Loader2 } from 'lucide-react'
  ...
  async function gerar() {
    ...
    try {
      const res  = await fetch('/api/gerar', {
        ...
        body: JSON.stringify({ tema, tom, qtdSlides: qtd }),
      })
      ...
      <div style={{ background:'#0D1117', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'2rem', marginBottom:'2rem' }}>
        ...
        <div style={{ marginBottom:'1.5rem' }}>
          ...
          <input
            ...
            placeholder="Ex: plano de saúde para MEI, seguro auto para Uber, remédio para emagrecer..."
          ...
          <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
            {TONS.map(t => (
              <button key={t} onClick={() => setTom(t)}
                style={{ padding:'8px 18px', borderRadius:'100px', border: tom === t ? '1px solid rgba(45,111,255,0.5)' : '1px solid rgba(255,255,255,0.07)', background: tom === t ? 'rgba(45,111,255,0.12)' : 'transparent', color: tom === t ? '#6B9FFF' : '#8B95A8', fontSize:'0.85rem', fontWeight:500, cursor:'pointer', fontFamily:'Sora, sans-serif', transition:'all 0.2s' }}>
                {TOM_LABELS[t]}
              </button>
            ))}
          </div>
        ...
        <div style={{ marginBottom:'1.5rem' }}>
          ...
          <input type="range" min={1} max={10} value={qtd} onChange={e => setQtd(Number(e.target.value))}
            style={{ width:'100%', accentColor:'#2D6FFF' }} />
        ...
        <button onClick={gerar} disabled={gerando}
          style={{ display:'flex', alignItems:'center', gap:'8px', background:'#2D6FFF', color:'#fff', border:'none', borderRadius:'10px', padding:'0.85rem 2rem', fontFamily:'Sora, sans-serif', fontWeight:600, fontSize:'0.95rem', cursor: gerando ? 'not-allowed' : 'pointer', opacity: gerando ? 0.7 : 1, boxShadow:'0 8px 24px rgba(45,111,255,0.3)', transition:'all 0.2s' }}>
      ...
      {slides.length > 0 && (
        <div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.5rem' }}>
            <h2 style={{ fontSize:'1.1rem', fontWeight:600, letterSpacing:'-0.02em' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            {slides.map((slide, idx) => (
              <div key={slide.id}
                style={{ background:'#0D1117', border: editando === slide.id ? '1px solid rgba(45,111,255,0.4)' : '1px solid rgba(255,255,255,0.07)', borderRadius:'14px', padding:'1.5rem', transition:'border-color 0.2s' }}>

                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'1rem' }}>
                  <div style={{ flex:1 }}>
                    {/* Número */}
                    <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:'0.65rem', color:'#4A5568', letterSpacing:'0.1em' }}>
                    ...
                    {editando === slide.id ? (
                      <input value={slide.titulo} onChange={e => editarSlide(slide.id, 'titulo', e.target.value)}
                        style={{ display:'block', width:'100%', background:'#080B12', border:'1px solid rgba(45,111,255,0.3)', borderRadius:'6px', padding:'6px 10px', color:'#F0F4FF', fontSize:'1rem', fontWeight:600, fontFamily:'Sora, sans-serif', outline:'none', marginTop:'6px', marginBottom:'8px', letterSpacing:'-0.02em' }} />
                    ) : (
          ...
          <div style={{ marginTop:'1.5rem', display:'flex', justifyContent:'flex-end' }}>
            <button onClick={baixarTudo} disabled={baixando}
              ...
              <Download size={16}/>
              {baixando ? 'Preparando ZIP...' : 'Baixar todos os slides'}
            </button>
          </div>
        </div>
      )}
